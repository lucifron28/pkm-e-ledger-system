...
export function DeleteTransactionForm({ id }: { id: string }) {
  ...
  return (
    <div className="inline">
      <button type="button" onClick={() => setIsOpen(true)} className="bg-red-100 hover:bg-red-200 text-red-700 font-bold px-2.5 py-1 rounded text-xs border border-red-300 transition">
        Delete
      </button>
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md p-6 space-y-4">
            ...
            <form action={formAction} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  name="deleteReason" required rows={3} placeholder="Reason for deleting this transaction..."
                ...
                />
                {state?.fieldErrors?.deleteReason && <p className="mt-1 text-xs text-red-600">{state.fieldErrors.deleteReason[0]}</p>}
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button type="submit" disabled={isPending} className="bg-red-600 hover:bg-red-700 text-white font-bold px-4 py-2 rounded-lg shadow transition text-sm disabled:opacity-50">
