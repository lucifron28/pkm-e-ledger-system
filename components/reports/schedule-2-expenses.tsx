        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            <thead className="bg-slate-100 text-slate-700 font-extrabold uppercase tracking-wider border-b-2 border-slate-300">
              <tr>
                <th className="px-2 py-2 text-left whitespace-nowrap">Doc No.</th>
                <th className="px-2 py-2 text-left whitespace-nowrap">Date</th>
                <th className="px-2 py-2 text-left whitespace-nowrap">Payee Name</th>
                <th className="px-2 py-2 text-left min-w-[120px]">Particulars</th>
                <th className="px-2 py-2 text-right whitespace-nowrap bg-red-100 text-red-900">Total Amount</th>
                {report.expenseCategories.map((cat) => (
                  <th key={cat.bucketKey} className="px-2 py-2 text-right whitespace-nowrap min-w-[75px]">
                    {cat.bucketName}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-200">
              {report.expenseRows.map((row) => (
                <tr key={row.transactionId} className="hover:bg-slate-50">
                  ...
                  <td className="px-2 py-2 text-right font-mono font-bold text-red-700 bg-red-50/50 whitespace-nowrap">
                    {formatPesoFromCents(row.amountCents)}
                  </td>
                  {report.expenseCategories.map((cat) => {
                    const bucketAmt = row.categoryBucketCents[cat.bucketKey] || 0;
                    return (
                      <td key={cat.bucketKey} className="px-2 py-2 text-right font-mono text-slate-700 whitespace-nowrap">
              ...
              <tr className="bg-slate-900 text-white font-extrabold border-t-2 border-slate-900">
                <td colSpan={4} className="px-2 py-3 text-right uppercase tracking-wider text-xs text-[#f9d818]">
                  Total Expenses:
                </td>
                <td className="px-2 py-3 text-right font-mono text-red-300 bg-slate-800 text-xs whitespace-nowrap">
                  {formatPesoFromCents(report.totalExpenseCents)}
                </td>
                {report.expenseCategories.map((cat) => (
                  <td key={cat.bucketKey} className="px-2 py-3 text-right font-mono text-[#f9d818] whitespace-nowrap">
